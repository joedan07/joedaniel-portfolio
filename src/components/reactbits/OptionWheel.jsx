...
const OptionWheel = ({
  items = DEFAULT_ITEMS,
  defaultSelected = 3,
  selected, // optional external sync: when this changes, the wheel follows without re-firing onChange
  onChange,
  textColor = '#a6a6a6',
  activeColor = '#ffffff',
  side = 'left',
  ...
  // External sync: follow the `selected` prop without echoing onChange back.
  useEffect(() => {
    if (selected == null) return;
    const cfg = cfgRef.current;
    const clamped = Math.min(Math.max(selected, 0), Math.max(cfg.count - 1, 0));
